...
describe('generate invite link', () => {
  test('should return INTERNAL_SERVER_ERROR when response is not ok', async () => {
    ...
    expect(result).toEqual({ data: response.INTERNAL_SERVER_ERROR });
    expect(global.fetch).toHaveBeenCalledWith(
      `https://discord.com/api/v10/guilds/channels/${dummyInviteBody.channelId}/invites`,
      {
        method: 'POST',
        headers: {
  ...
  test('should return JSON response when response is ok', async () => {
    ...
    expect(result).toEqual({ ...mockResponse, message: response.INVITED_CREATED });
    expect(global.fetch).toHaveBeenCalledWith(
      `https://discord.com/api/v10/guilds/channels/${dummyInviteBody.channelId}/invites`,
      {
        method: 'POST',
